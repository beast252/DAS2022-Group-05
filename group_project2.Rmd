...
# formal analysis
## change the votes variable to log votes
```{r}
film <- film %>%
...
```{r}
model <-  glm(rating.large7 ~ year + length + budget + log.votes + genre, family = binomial(link = "logit"), data = film)
...
|:-----------------|----:|---:|----:|
|model1     | $g(p_i)=log(\frac{p_i}{1-p_i})$| 1303.21|  1360.60|
|model2     | $g(p_i)=\phi^{-1}(pi),p_i=\phi(\frac{x_i-\mu}{\sigma})$ | 1328.18|1385.57|
|model3     | $g(p_i)=log[-log(1-p_i)]$ | 1402.13|1459.51|

D0-D1 = 2982.5-2357.5 = 1692.401 >  x^2(0.95, 9) = 16.91898, so we reject H0, we can say that the model1 fits the data better than saturated model.

$$D_0-D_1 = 2982.5-2357.5 = 1692.401 >  {\chi}^2(0.95, 9) = 16.91898 $$ 
So we reject H0, and we can say that the model1 fits the data better than saturated model.
...
AUC = 0.948 indicated that model 1 is very good at predicting the films rating greater than 7 given all predictor variables. 
