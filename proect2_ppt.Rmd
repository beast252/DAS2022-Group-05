...
```{r}
model_sat <-  glm(rating.large7 ~ length + budget + genre + log.votes + year, family = binomial(link = "logit"), data = film.noRomance)
...
```{r}
model1 <- glm(rating.large7 ~ length + budget + log.votes + genre, family = binomial(link = "logit"), data = film.noRomance)
...
```{r}
model2 <- glm(rating.large7 ~ length + budget + log.votes + genre, family = binomial(link = "probit"), data = film.noRomance)
summary(model2)$coefficients%>%kable()
```
...
```{r}
model3 <- glm(rating.large7 ~ length + budget + log.votes + genre, family = binomial(link = "cloglog"), data = film.noRomance)
summary(model3)$coefficients%>%kable()
```
...
```
```{r}
df_nested = nrow(film.noRomance) - length(model1$coefficients)
```
...
## ROC curve and AUC
```{r}
film.noRomance$Prid <- predict(model1, film.noRomance, type="response")
score <- prediction(film.noRomance$Prid,film.noRomance$rating.large7)
...
```{r}
film1 <- film%>%
  mutate(rating.large5.5 = cut(rating, breaks = c(0,5.5,Inf), labels=c(0,1)),
         log.votes = log(votes))
```

```{r}
model_rating5.5 <- glm(rating.large5.5 ~ length + budget + log.votes + genre, family = binomial(link = "logit"), data = film1)
summary(model_rating5.5)$coefficients%>%kable()
```

```{r}
plot_model(model_rating5.5,type="pred",terms=c("genre"),axis.title=c("genre","Prob(rating above 5.5)"))
```
